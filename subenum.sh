...
cd ~/tools
dnsvalidator -tL https://public-dns.info/nameservers.txt -threads 50 --silent | tee -a resolvers-valid-all.txt
...
puredns bruteforce $Wordlist flipkart.com -r ~/tools/Wordlists/resolvers-valid-all.txt --bin ~/tools/massdns/bin/massdns --write valid_domains.txt --write-wildcards wc_domains.txt --write-massdns massdns.txt